...
biogrid = read_tsv("./data/BIOGRID-ALL-3.5.174.mitab.txt.gz")
...
get_msig_slopes = function(sub_cats, database) {
  ...
  for (j in unique(msig$gs_name)) {
    ...
    diseases_curr = diseases2frame("temp_rea_bio.txt", links = biogrid) # [??? Shouldn't it be database?]
    #Get disease name
...
all_cats = subcats[which(subcats!="")]
...
The above function takes internal and external edges from each gene for each disease and calculates the total number of genes and the slope of internal vs. external edges. Genes with zero internal edges are filtered and diseases with less than 5 non-zero internal edges are filtered.
...
```{r}
# [??? What this function does? Write roxygen help]
calc_norm_coherence = function(msig_frame, dis_frame, database, num_msig = 10, num_rand = 10) {
