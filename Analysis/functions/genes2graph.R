genes2graph = function(gene_list, actions, directed = TRUE) {
  ...
  act_sub = actions %>% 
    filter(hgnc_symbol_a %in% gene_list | hgnc_symbol_b %in% gene_list) %>% mutate(hgnc_acting = ifelse(a_is_acting == "t",hgnc_symbol_a, hgnc_symbol_b),
                                                                                   hgnc_receiving = ifelse(a_is_acting == "t",                                                                                                        hgnc_symbol_b, hgnc_symbol_a)) %>% dplyr::select(hgnc_acting, hgnc_receiving) %>% filter(complete.cases(.))
