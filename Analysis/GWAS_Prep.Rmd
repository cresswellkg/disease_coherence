...
write.csv(init_sum, "supplementary_table_diseases_raw.csv", row.names = FALSE)
...
efo = getOntology(efo_loc)
# save(list = "efo",file =  "./data/efo.rdata")
# 
# load("./data/efo.rdata")
...
```{r}
#Creating KEGG pathways

mart  = useMart("ensembl", dataset="hsapiens_gene_ensembl")
min_genes = 5

xx = as.list(KEGGPATHID2EXTID)     # All KEGG pathways
xx = xx[ grepl("hsa", names(xx)) ] # Keep hsa pathways only
xx_length = sapply(xx, length)     # Number of genes in each pathway
xx = xx[xx_length > min_genes]     # Keep pathways with minimum number of genes or higher
xx_sample = sample(1:length(xx), size = length(xx), replace = FALSE) # reshuffled indices
# Get all gene symbols for KEGG pathways
genes = getBM(attributes=c('entrezgene', 'hgnc_symbol'), filters='entrezgene', values=unique(unlist(xx)), mart=mart, uniqueRows=T)

kegg_paths = lapply(xx, function(x) left_join(data.frame(entrezgene = as.integer(x)), genes))

#Loop through kegg pathways and save genes 

dir.create("./data/Disease_Genes/KEGG")

for (i in 1:length(kegg_paths)) {
  genes = kegg_paths[[i]][,2]
  write.table(genes, paste0("./data/Disease_Genes/KEGG/", names(kegg_paths)[i] ), row.names = FALSE, col.names = FALSE)
