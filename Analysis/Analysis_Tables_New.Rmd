...
print(paste0("Average correlation with network size: ", mean(diff_cors[grepl("Number", rownames(diff_cors)), grepl("Coherence", colnames(diff_cors))])))
# Average correlation of coherences
diff_cors_selected <- diff_cors[grepl("Coherence", rownames(diff_cors)) & grepl("String", rownames(diff_cors)), grepl("Coherence", colnames(diff_cors)) & grepl("String", rownames(diff_cors))]
...
png(filename = "../manuscript/Figures/FigS4_summary_diseases_Biogrid.png", height = 8, width = 14, units = "in", res = 300)
lay1 <- rbind(c(1, 2, 3, 4))
lay2 <- rbind(c(1, 2, 3, 4))
g1 <- arrangeGrob(grobs = list(p_intelligence, p_anthropometric, p_neurologic, p_psychiatric), layout_matrix = lay1)
g2 <- arrangeGrob(grobs = list(p_cardiovascular, p_metabolic, p_eye), layout_matrix = lay2)
